...
**Erase model:** Implicit auto-erase — each page write erases the target page internally before programming. No explicit chip-level erase command. Chip-level erase is available via a 6-cycle SDP sequence (0x80→0x5555 then 0x10→0x5555) on chips that support it. The write path (`flash_5v_page_write_init`) performs **no blank check** on this protocol either (Phase 153, ERASE-02), for the same reason as `0x0D` above — the silicon auto-erases per page during the write, so a pre-write blank check was a false precondition rather than a safety net. The capability-gated bulk erase described above is unchanged and remains **deliberately unadvertised** by the host: setting `FLAG_CAN_ERASE` for algorithm 5 would route a 12 V bulk erase onto this 5V-only part — a live hardware-hazard exclusion, distinct from algorithm 13's now-retired reason above.
