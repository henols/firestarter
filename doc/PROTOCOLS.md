...
**Erase model:** Firmware implements **no erase operation at all** for protocol `0x0D` — `configure_eeprom28c()` (`eeprom_28c.cpp`) exposes no erase entry point of any kind. The only erase-like behavior is the implicit per-page auto-erase that happens internally as part of every page write; there is no separate chip-erase or sector-erase command on this path, on any part in the bucket. The host's general erase-capability advertisement (`FLAG_CAN_ERASE`, derived from `electrical.type == "EEPROM"`/`"Flash/EEPROM"`) is authoritative for every other protocol in this document, but is **deliberately not set for protocol `0x0D`**: the host clears the flag for all 84 chips in this bucket (Phase 121 D-12) precisely because there is nothing here to advertise. Operational consequence for `firestarter write`: `--skip-erase` has nothing to skip on a `0x0D` chip and only warns and proceeds; the flag that actually matters on this family is the blank-check skip (`-b`/`--no-blank-check`), which is **required** to write a non-blank AT28C part — because there is no erase step to make it blank first — and which, on this family only, skips the blank check and nothing else.
