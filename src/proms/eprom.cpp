...
// error_code stays a parameter here, not a mem_util_report_chip_id-internal
// is_flag_set(FLAG_FORCE) test: this function's two callers disagree on
// policy by design -- eprom_check_chip_id_execute (the standalone
// CMD_CHECK_CHIP_ID entry) passes RESPONSE_CODE_ERROR unconditionally, while
// eprom_generic_init passes a FLAG_FORCE-derived value -- so the standalone
// chip-ID check refuses regardless of --force. Do not "finish" this
// de-duplication by folding the flag test inward.
void eprom_internal_check_chip_id(firestarter_handle_t* handle, uint8_t error_code) {
    LOG_DEBUG_ID_SUB(DBG_CHECK_CHIP_ID);
