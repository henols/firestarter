...
<!-- Frozen copy of .planning/REQUIREMENTS.md as it stood at the v1.41 close (2026-09-24).
     The live file stays in place for /gsd-new-milestone to replace — the `git rm` step
     stopped being used after v1.35. Outcome: 34/34 Complete; none dropped, none deferred.
     Three requirements were amended in place during the milestone by recorded operator
     decision, and each amendment names its decision and the text it replaced: CMP-04 (D-13,
     no byte values printed), FWCMD-05 (D-03, the ids each verify actually raises),
     WRITE-02 (D-01/D-02, the guarded set pinned both ways). The close repointed six
     `phases/20x-…` citations to `milestones/v1.41-phases/`. -->
