...
# Milestone Archive: v1.34 ROADMAP — Pre-Merge Hardware Regression Validation

**Status:** ⚠ CLOSED 2026-08-29 — **EARLY / SCOPE-REDUCED. This is not a completed sweep.**
**Phases:** 160–166 authored (7). **4 complete** (160, 161, 165, 166) · **1 partial** (162, at 5 of 11 chips) · **2 NOT RUN** (163 SHIELD, 164 REV0). 28 plans authored, 24 completed.
**Archived by:** hand, on 2026-08-29, following the `/gsd-complete-milestone` shape used for v1.31–v1.33.
**Closeout type:** `operator_directed_early_close` — the operator directed "stop testing, close ASAP" on 2026-08-29 and the milestone closed on the evidence already banked. Not a gap in the work that ran.
**Verdict:** **MERGE WITH CAVEATS** — see `.planning/v1.34/CLOSE-RECORD.md` §4 for the five caveats and §5 for the ten-row honesty ledger.
**Headline non-claim:** program-window VPP/VCC **under load** was never measured. **v1.34 makes no electrical claim whatsoever.**
**Coverage warning, stated here so it cannot be missed:** every v1.34 result is **Rev 2.0 only**. Six of eleven chips and two of three shields were never swept.

This is a verbatim snapshot of `.planning/ROADMAP.md` as it stood at the v1.34 close, taken **after**
the close-time edits were applied. The live `ROADMAP.md` retains its own `## v1.34` section; this file
is the frozen copy the milestone entry points at. Per this project's standing rule, `ROADMAP.md` is
**hand-authored and was never regenerated** — the close edited exactly three regions of it (the
Milestones-list bullet plus its two appended close paragraphs, the `## v1.34` section heading, and the
Phase 162–166 checkboxes), plus one in-place annotation on Phase 166's criterion 4 recording the
CLOSE-04 deviation.

**⚠ ONE CITATION IN THIS SNAPSHOT IS KNOWN-WRONG AND DELIBERATELY RETAINED.** Phase 164's criterion 2
cites the upstream Rev 0 schematic as blob `d2a7f691`. It is not: `d2a7f691` is
`hardware/W27C512Programmer.kicad_sch` on `origin/rev2.0`, and the true Rev 0 schematic is blob
`cfe6139f` at commit `486f3d1` (established post-close by `0e114fb7`). Phase 164 closed unrun, so the
criterion is kept as inert history and annotated in place rather than rewritten.
