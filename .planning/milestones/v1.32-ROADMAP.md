...
**Scoped from a root-cause pass, not from the issue text.** `devtest-triage` had already cleared the AT28C256 *data* against Atmel/Microchip DS20006386B — all 28 pins of `DIP28_28C256` agree, `infoic_page_size_raw: 64` is the datasheet page register, `chip_id_check: false` is correct — and handed the question on as host/firmware. The root-cause pass then found why that question is currently unanswerable: **`cli_handlers.py:2500` hardcodes `fw_board_identity=None`**, so **every `dev test` report ever filed carries a null firmware identity**. gh#21 and gh#32 report host `3.0.0b15` against an unknown firmware and cannot be distinguished from a board lacking the entire Phase-117–120 `0x0D` fix stack (FIX-01 `/WE`-inhibit routing, FIX-03 A16–A18 staleness, FIX-06 the completion-vs-data-landed conflation that is gh#11's actual shape).
...
**Success Criteria** (what must be TRUE):
  ...
  1. The pre-write blank check is gone on both auto-erasing protocols: the one conditional at `firestarter/src/proms/eeprom_28c.cpp:517` (`if (!is_flag_set(FLAG_SKIP_BLANK_CHECK)) { mem_util_blank_check(handle); }`) and its `flash_5v_page.cpp` sibling — the latter **located in code, not assumed to exist** by symmetry.
  2. `erase` works as a standalone step on `0x0D`: a `CMD_ERASE` arm exists in `configure_eeprom28c`, and `FLAG_CAN_ERASE` is restored for `algorithm 13` at `firestarter_app/firestarter/database.py:629`.
  ...
  4. `blank` remains available as its own step — `cli_handlers.py:854` → `CMD_BLANK_CHECK` → `mem_util_blank_check` **already works and nothing is owed there**; this is a non-regression assertion, not new work.
  5. `info`'s "can be erased" row (`firestarter_app/firestarter/ic_layout.py:579`) agrees with the wire flag instead of contradicting it.
  6. The stale Phase 121 D-12 **code comment** at `firestarter_app/firestarter/database.py:591` is corrected (152-CONTEXT.md **D-15**) — this phase owns it because it must touch `database.py:629` anyway, which keeps Phase 152 out of a sub-repo for a comment edit.
