...
**Current focus:** v1.41 — all six phases (202–207) complete 2026-09-23. Phase 207 CLOSED: `3.1.0b1` in both sub-repos in one commit pair on top of a local merge of `origin/beta` (unpushed — the ship PR publishes), wiki record live at `880a59b`. Outstanding before the close: `/gsd-secure-phase 206` and `/gsd-secure-phase 207` (security enforcement on), WR-01 (live Testing-Chips page says `dev test` runs twice; default is three), then `/gsd-audit-milestone` and a hand-archived close. Dual-repo lockstep on branch `v1.41-verification-to-host` in all three repos. **v1.40 is still unmerged** — `henols/firestarter#91`, `henols/firestarter_app#72`, `henols/firestarter_fw#70`.
