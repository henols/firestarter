...
- **Serial protocol changes must be kept in sync** between `firestarter_app/firestarter/serial_comm.py` and `firestarter_fw/src/firestarter.cpp`. Never change one alone.
- **Constants and flag bits are duplicated** between `firestarter_app/firestarter/constants.py` (Python) and `firestarter_fw/include/firestarter.h` (C++). Change both together.
...
- `platformio.ini` at the repo root — generated by `.devcontainer/gen-platformio-ini.py` from `firestarter_fw/platformio.ini`; gitignored. Banner: `; Auto-generated ... do not edit manually.`
...
convention throughout `firestarter_fw/`, not an occasional departure from the pre-raise
