 ...
 * PROGMEM log-tag globals from src/logging.c, so the test_pinmap_provisional
 * test binary can link.
 *
 * Suite-specific extensions: NONE — test_pinmap_provisional uses the
 * canonical default for every stub, so this TU is a pure pass-through to
 * the shared include.
 *
 * Scope: only compiled into [env:native_pinmap_provisional] via PIO's
 * automatic discovery of files under test/. Production builds (env:uno,
 * env:leonardo) never see this file because their src_filter excludes
 * test/.
 */
